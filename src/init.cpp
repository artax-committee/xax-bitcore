...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/artax-committee/xax>";
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                if (fReindex) {
                    boost::filesystem::path stateDir = GetDataDir() / "stateXax";
                ...
                boost::filesystem::path xaxStateDir = GetDataDir() / "stateXax";

                bool fStatus = boost::filesystem::exists(xaxStateDir);
                const std::string dirXax(xaxStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsXaxstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<XaxState>(new XaxState(dev::u256(0), XaxState::openDB(dirXax, hashDB, dev::WithExisting::Trust), dirXax, existsXaxstate));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::xaxMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult = new StorageResults(xaxStateDir.string());
