...
bool ToHash160(const std::string& strXaxAddress, std::string& strHash160)
{
    CBitcoinAddress xaxAddress(strXaxAddress);
    if(xaxAddress.IsValid()){
        CKeyID keyid;
        xaxAddress.GetKeyID(keyid);
...
bool ToXaxAddress(const std::string& strHash160, std::string& strXaxAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CBitcoinAddress xaxAddress;
    xaxAddress.Set(keyid);
    if(xaxAddress.IsValid()){
        strXaxAddress = xaxAddress.ToString();
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToXaxAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToXaxAddress(tokenEvent.receiver, tokenEvent.receiver);
