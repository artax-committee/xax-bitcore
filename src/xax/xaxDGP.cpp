...
bool XaxDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule XaxDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t XaxDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t XaxDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t XaxDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t XaxDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool XaxDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void XaxDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void XaxDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void XaxDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void XaxDGP::createParamsInstance(){
...
dev::Address XaxDGP::getAddressForBlock(unsigned int blockHeight){
...
void XaxDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void XaxDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void XaxDGP::parseStorageOneUint64(uint64_t& value){
...
void XaxDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule XaxDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
