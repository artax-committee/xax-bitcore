...
XaxState::XaxState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = XaxState::openDB(_path + "/xaxDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

XaxState::XaxState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute XaxState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, XaxTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createXaxAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
...
std::unordered_map<dev::Address, Vin> XaxState::vins() const // temp
{
...
void XaxState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* XaxState::vin(dev::Address const& _a) const
{
    return const_cast<XaxState*>(this)->vin(_a);
}

Vin* XaxState::vin(dev::Address const& _addr)
{
...
// void XaxState::commit(CommitBehaviour _commitBehaviour)
// {
...
void XaxState::kill(dev::Address _addr)
{
...
void XaxState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
const dev::Address XaxState::createXaxAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void XaxState::deleteAccounts(std::set<dev::Address>& addrs){
...
void XaxState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void XaxState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
