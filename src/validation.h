...
extern std::unique_ptr<XaxState> globalState;
...
using ExtractXaxTX = std::pair<std::vector<XaxTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashXaxRoot,
    const dev::h256& oldHashStateRoot, const std::vector<XaxTransaction>& transactions);
...
class XaxTxConverter{

public:

    XaxTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionXaxTransactions(ExtractXaxTX& xaxTx);
...
private:
    ...
    XaxTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<XaxTransaction> _txs, const uint64_t _blockGasLimit) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit) {}
