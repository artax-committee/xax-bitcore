...
#if defined(NDEBUG)
# error "Xax cannot be compiled without assertions."
#endif
...
std::unique_ptr<XaxState> globalState;
...
const std::string strMessageMagic = "Xax Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > MAX_TRANSACTION_BASE_SIZE ||
        ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // xax
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            XaxDGP xaxDGP(globalState.get(), fGettingValuesDGP);
            ...
            XaxTxConverter converter(tx, NULL);
            ExtractXaxTX resultConverter;
            if(!converter.extractionXaxTransactions(resultConverter)){
            ...
            for(XaxTransaction xaxTransaction : xaxTransactions){
                ...
                if(xaxTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s XAX additional fees, %d delta bytes\n",
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    XaxTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<XaxTransaction>(1, callTransaction), blockGasLimit);
...
bool XaxTxConverter::extractionXaxTransactions(ExtractXaxTX& xaxtx){
    std::vector<XaxTransaction> resultTX;
...
bool XaxTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool XaxTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
XaxTransaction XaxTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    XaxTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = XaxTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = XaxTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // xax
    XaxDGP xaxDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setXaxSchedule(xaxDGP.getGasSchedule(pindex->nHeight + 1));
...
///////////////////////////////////////////////////////////////////////////////////////// xax
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            XaxTxConverter convert(tx, &view, &block.vtx);

            ExtractXaxTX resultConvertXaxTX;
            if(!convert.extractionXaxTransactions(resultConvertXaxTX)){
            ...
            if(!CheckMinGasPrice(resultConvertXaxTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertXaxTX.first, blockGasLimit);
            ...
            for(XaxTransaction& qtx : resultConvertXaxTX.first){
            ...
            if (fLogEvents)
            {
                for(size_t k = 0; k < resultConvertXaxTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertXaxTX.first[k].from(), resultConvertXaxTX.first[k].to(),
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Xax PoS we need to sync this after every block to ensure txdb is populated for
