...
void initState(){
    ...
    const std::string dirXax = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<XaxState>(new XaxState(dev::u256(0), XaxState::openDB(dirXax, hashDB, dev::WithExisting::Trust), dirXax + "/xaxDB", dev::eth::BaseState::Empty));
...
dev::Address createXaxAddress(dev::h256 hashTx, uint32_t voutNumber){
...
XaxTransaction createXaxTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout){
    XaxTransaction txEth;
    if(recipient == dev::Address()){
        txEth = XaxTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = XaxTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<XaxTransaction> txs){
