...
void checkResult(bool isCreation, std::vector<XaxTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    XaxTxConverter converter(transaction, NULL);
    ExtractXaxTX xaxTx;
    BOOST_CHECK(converter.extractionXaxTransactions(xaxTx));
    std::vector<XaxTransaction> result = xaxTx.first;
    if(script2 == CScript()){
