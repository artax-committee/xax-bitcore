...
void OpenDebugLog()
{
    ...
    assert(fileout == NULL);
    assert(fileoutVM == NULL); // xax
    assert(vMsgsBeforeOpenLog);
    boost::filesystem::path pathDebug = GetDataDir() / "debug.log";
    boost::filesystem::path pathDebugVM = GetDataDir() / "vm.log"; // xax
    fileout = fopen(pathDebug.string().c_str(), "a");
    fileoutVM = fopen(pathDebugVM.string().c_str(), "a"); // xax
    if (fileout) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Xax
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Xax
    // Mac: ~/Library/Application Support/Xax
    // Unix: ~/.xax
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Xax";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Xax";
#else
    // Unix
    return pathRet / ".xax";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Xax Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Xax Core developers";
    }
