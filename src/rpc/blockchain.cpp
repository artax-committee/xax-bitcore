...
{
    if (pindexBestHeader->nHeight >= Params().GetConsensus().nLastPOWBlock)//tonixax edit
        return 0;
...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.push_back(Pair("hashStateRoot", blockindex->hashStateRoot.GetHex())); // xax
    result.push_back(Pair("hashUTXORoot", blockindex->hashUTXORoot.GetHex())); // xax
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    result.push_back(Pair("hashStateRoot", block.hashStateRoot.GetHex())); // xax
    result.push_back(Pair("hashUTXORoot", block.hashUTXORoot.GetHex())); // xax
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() == 3){
        CBitcoinAddress xaxSenderAddress(request.params[2].get_str());
        if(xaxSenderAddress.IsValid()){
            CKeyID keyid;
            xaxSenderAddress.GetKeyID(keyid);
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "blockchain",         "callcontract",           &callcontract,           true,  {"address","data"} }, // xax
